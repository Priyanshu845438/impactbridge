...
const navLinks = [
  ...
  { href: "/dashboard/ngo", label: "NGO", roles: ["NGO"] },
  { href: "/dashboard/company", label: "Company", roles: ["COMPANY"] },
  { href: "/dashboard/donor", label: "Donor", roles: ["DONOR"] },
];
...
export default function DashboardLayout({ children }: PropsWithChildren) {
  ...
  return (
    <div className="w-full min-h-screen flex overflow-hidden bg-slate-50">
            ...
            {availableLinks.map((link) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === link.href
                    ? "bg-emerald-400/90 text-slate-900 shadow-lg shadow-emerald-500/30"
      ...
      <div className="ml-[260px] w-[calc(100%-260px)] flex flex-col">
        <header className="sticky top-0 z-40 flex items-center justify-between border-b border-slate-200 bg-white/90 px-4 py-4 shadow-sm shadow-slate-200 backdrop-blur sm:px-6 lg:px-10">
          <button
            type="button"
      ...
      {mobileOpen ? (
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50 px-6 py-6 lg:px-10 lg:py-10">
