...
| `API_PROGRAMME` | `NEXT_PUBLIC_FLAG_API_PROGRAMME` | `false` | Enables company CSR programme list/detail/create/update/status/assignment flows to consume backend data via React Query wrappers. When `true`, pages call `/companies/{id}/csr-programmes` endpoints for reads and mutations (create/update/status/assign); when `false`, all flows fall back to mocks. Contract tests guarantee identical UI output for flag on/off, loading, and error states. |
