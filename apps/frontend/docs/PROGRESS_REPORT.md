...
_Last updated: 2025-12-18 08:20 UTC_
...
- Performance budgets defined for top dashboard routes with Next metrics hook emitting warnings only; baseline recorded in `docs/PERFORMANCE_BASELINE.md`.
- Documented server-driven navigation contract and mapping utility; added Jest coverage to validate role merging, ordering, and icon handling ahead of backend wiring.
...
- Added company impact story management page with search, filters, sort controls, responsive card grid, status dropdown workflow, tag selector, timeline note, skeleton loaders, empty state, and builder CTA (route: `/dashboard/company/impact-stories/manage`).
