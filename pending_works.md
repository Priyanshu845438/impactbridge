...
## 3. apps/frontend
1. CSR programme list/detail pages use feature-flagged React Query wrappers with shared query keys, ensuring cache invalidation stays in sync after create/update/status/assignment mutations. Runtime API fallback logic is still unimplemented, so page-level RTL parity tests and end-to-end validation (RBAC, pagination) remain pending before enabling the flag by default.
