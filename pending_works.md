...
## 3. apps/frontend
1. CSR programme list/detail pages now use feature-flagged React Query wrappers that call the live API when `API_PROGRAMME` is enabled, with mocks as fallback; detail hook fallback paths are unit-tested but before default-on launch we still need end-to-end validation (loading/error UX, RBAC) and rollout coordination.
