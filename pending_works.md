...
4. CSR service integration tests exist only as in-memory workflow specs; HTTP contract coverage now exists in `apps/backend/__tests__/csr-programme/csr-programme.routes.spec.ts` and the new happy-path e2e suite, but RBAC permutations, negative paths, and legacy `/api/v1` e2e suites still fail pending Prisma-backed fixtures.
