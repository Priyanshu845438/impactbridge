...
## 3. apps/frontend
1. CSR programme list/detail pages now use feature-flagged React Query wrappers that call the live API when `API_PROGRAMME` is enabled, with mocks as fallback; list pagination mismatches are normalised but we still need end-to-end validation (loading/error UX, RBAC, pagination parity) and rollout coordination before flipping the flag by default.
